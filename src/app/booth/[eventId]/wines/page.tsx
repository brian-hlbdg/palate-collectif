...
export default function BoothWinesPage() {
  ...
  const [isTempUser, setIsTempUser] = useState(true) // Booth users are always temp
  ...
  const checkUser = async () => {
    // For booth mode, check localStorage
  ...
  const loadData = async () => {
    ...
    if (!currentUserId) {
      // Redirect to booth entry if no user
  ...
  // Filter wines based on search and type
    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-[var(--surface)]/95 backdrop-blur-xl border-t border-[var(--border)] z-30">
        <div className="max-w-2xl mx-auto flex items-center justify-around py-2 px-4 safe-area-inset-bottom">
          ...
          {/* Results Button (shows after event closes) OR For You */}
          {eventClosed ? (
            <button
          ...
          <button
            onClick={() => router.push(`/booth/${eventCode}/profile`)}
            className="flex flex-col items-center gap-1 px-3 py-2 rounded-xl text-[var(--foreground-secondary)] hover:text-[var(--foreground)] transition-colors"
          >

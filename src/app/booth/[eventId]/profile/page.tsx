...
export default function BoothProfilePage() {
  ...
  const sortedWines = [...ratedWines].sort((a, b) =>
    sortBy === 'rating' ? b.rating - a.rating : a.tasting_order - b.tasting_order
  )
        ...
        {ratedWines.length === 0 ? (
          ...
          <>
            ...
            {topWine && (
              <Card variant="elevated" padding="md" className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-body-sm font-semibold text-[var(--gold)]">Your Top Pick</span>
                </div>
                <Link href={`/booth/${eventCode}/rate/${topWine.id}`}>
            ...
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-body-lg font-semibold text-[var(--foreground)]">Your Ratings</h2>
              <div className="flex gap-1 p-1 rounded-xl bg-[var(--background)]">
                <button
                  onClick={() => setSortBy('rating')}
                  className={cn(
                    'px-3 py-1.5 rounded-lg text-body-sm font-medium transition-colors duration-200',
                    sortBy === 'rating' ? 'bg-[var(--wine)] text-white' : 'text-[var(--foreground-secondary)]'
                  )}
                >
                  By Rating
                </button>
                <button
                  ...
                  className={cn(
                    'px-3 py-1.5 rounded-lg text-body-sm font-medium transition-colors duration-200',
                    sortBy === 'order' ? 'bg-[var(--wine)] text-white' : 'text-[var(--foreground-secondary)]'
                  )}
                >
                  By Order
                </button>
            ...
            <div className="space-y-3">
              {sortedWines.map((wine, index) => (
                ...
                >
                  <Link href={`/booth/${eventCode}/rate/${wine.id}`}>
